...
interface PaymentFormProps {
  ...
  timeSlot: string;
  onBack: () => void;
  onSubmit: (details: { name: string, phone: string, proof: string }) => void;
}

export default function PaymentForm({ service, vehicleSize, fuelType, date, timeSlot, onBack, onSubmit }: PaymentFormProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const fakeUrl = URL.createObjectURL(proofFile);
    onSubmit({ name, phone, proof: fakeUrl });
  };
      ...
      <div className="lg:col-span-3">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex justify-between pt-6 border-t">
            <button type="button" onClick={onBack} className="px-6 py-3 font-bold text-gray-500 hover:text-gray-900">
              BACK
            </button>
            <button 
              type="submit"
              disabled={!name || !phone || !proofFile}
              className={`px-8 py-3 rounded-lg font-bold text-white transition-colors ${
                name && phone && proofFile ? 'bg-orange-600 hover:bg-orange-700' : 'bg-gray-300 cursor-not-allowed'
              }`}
            >
              COMPLETE BOOKING
            </button>
